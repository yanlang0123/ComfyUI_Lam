...
class Segment_animations:
    ...
    def get_animation(self,animation_type):
        anis=[animation for animation in self.animations if animation.animation_type==animation_type]
        return anis[0] if anis else None
    ...
    def add_animation(self, animation: Union[Video_animation, Text_animation]) -> None:
        ...
        if isinstance(animation, Video_animation):
            ...
            if animation.animation_type == "group" and len(self.animations) > 0:
                #根据类型获取入场动画没有返回none
                inani=self.get_animation("in")
                outani=self.get_animation("out")
                if inani:
                    animation.start = inani.start+inani.duration
                if outani and outani.start<animation.start+animation.duration:
                    animation.duration=outani.start-animation.start
                if animation.duration<=0:
                    raise ValueError("组合动画无有效时间，请调整入场动画或者出场动画")
            elif animation.animation_type == "in":
                groupani=self.get_animation("group")
                outani=self.get_animation("out")
                if groupani:
                    groupani.start=animation.start+animation.duration
                if outani and groupani and outani.start<groupani.start+groupani.duration:
                    groupani.duration=outani.start-groupani.start
                    if groupani.duration<=0:
                        raise ValueError("组合动画无有效时间，请调整入场动画或者出场动画")
            elif animation.animation_type == "out":
                groupani=self.get_animation("group")
                if groupani and animation.start<groupani.start+groupani.duration:
                    groupani.duration=animation.start-groupani.start
                    if groupani.duration<=0:
                        raise ValueError("组合动画无有效时间，请调整入场动画或者出场动画")
