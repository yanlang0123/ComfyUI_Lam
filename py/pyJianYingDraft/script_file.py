...
class Script_file:
    ...
    TEMPLATE_INFO_FILE = "draft_meta_info.json"

    def __init__(self,name: str, width: int, height: int, fps: int = 30,draft_root: str = r"C:\Users\Administrator\AppData\Local\JianyingPro\User Data\Projects\com.lveditor.draft",):
        """创建一个剪映草稿
        ...
        """
        self._drafts_root = draft_root
        self._draft_folder = os.path.join(self._drafts_root, name)
        self.save_path = os.path.join(self._drafts_root, name,self.TEMPLATE_FILE)
        self.save_info_path = os.path.join(self._drafts_root, name,self.TEMPLATE_INFO_FILE)
        ...
        template_folder = os.path.join(here, "template")

        with open(os.path.join(template_folder, self.TEMPLATE_FILE), "r", encoding="utf-8") as f:
            self.content = json.load(f)
        with open(os.path.join(template_folder, self.TEMPLATE_INFO_FILE), "r", encoding="utf-8") as f:
            self._draft_meta_info_data = json.load(f)

        self._materials_in_draft_meta_info: {} = self._draft_meta_info_data['draft_materials']  # 草稿元数据库的素材
        self._all_material_in_draft_meta_info = self._materials_in_draft_meta_info[0]['value']
    ...
    def add_segment(self, segment: Union[Video_segment, Sticker_segment, Audio_segment, Text_segment],
        ...
        if isinstance(segment, Video_segment) or isinstance(segment, Audio_segment):
            self._all_material_in_draft_meta_info.append(segment.data_for_meta_info)
    ...
    def dumps(self) -> str:
        """将草稿文件内容导出为JSON字符串"""
        self.content['id'] = tools.generate_id()
    ...
    def dumps_info(self) -> str:
        ...
        return json.dumps(self._draft_meta_info_data, ensure_ascii=False, indent=4)

    def save(self) -> None:
        #路径不存在创建
        if os.path.exists(self._draft_folder)==False:
            os.makedirs(self._draft_folder)
        """将草稿文件内容写入文件"""
        with open(self.save_path, "w", encoding="utf-8") as f:
            f.write(self.dumps())
        with open(self.save_info_path, "w", encoding="utf-8") as f:
            f.write(self.dumps_info())
